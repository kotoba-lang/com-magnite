(ns magnite.main
  "Kotodama WASM entrypoint for the Magnite clean-room actor (L4) — Clojure port.
  ...
  facts (`magnite.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "magnite")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"magnite.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "magnite-compat" :tier tier :entities entities} 200])
...
(defn start! [] :magnite-compat/ready)
